...
#define GLOBAL_CONF_FLASH_ADDR    0
#define CONF_SET_0_FLASH_ADDR     GLOBAL_CONF_FLASH_ADDR + sizeof(global_conf_t)
#define CONF_SET_1_FLASH_ADDR     CONF_SET_0_FLASH_ADDR + sizeof(conf_t)
#define CONF_SET_2_FLASH_ADDR     CONF_SET_1_FLASH_ADDR + sizeof(conf_t)


void eeprom_read_block (void *__dst, uint32_t flashAddrOfset, uint32_t __n)
{
  byte* b = dueFlashStorage.readAddress(flashAddrOfset); // byte array which is read from flash at adress 4
  memcpy(__dst, b, __n); // copy byte array to temporary struct  
}

void eeprom_write_block (void *__src, uint32_t flashAddrOfset, uint32_t __n)
{
  dueFlashStorage.write(flashAddrOfset, (byte*)__src, __n); // write byte array to flash at address 4 
}
...
void readGlobalSet() {
#if defined (ARDUINO_DUE)
  eeprom_read_block((void*)&global_conf, GLOBAL_CONF_FLASH_ADDR, sizeof(global_conf));
...
bool readEEPROM() {
  ...
  eeprom_read_block((void*)&conf, CONF_SET_0_FLASH_ADDR + global_conf.currentSet * sizeof(conf), sizeof(conf));   
#else
...
void writeGlobalSet(uint8_t b) {
  ...
  eeprom_write_block((void*)&global_conf, GLOBAL_CONF_FLASH_ADDR, sizeof(global_conf));
#else
...
void writeParams(uint8_t b) {
 ...
 //Serial.println("writeParams");
   eeprom_write_block((void*)&conf, CONF_SET_0_FLASH_ADDR + global_conf.currentSet * sizeof(conf), sizeof(conf));  
 #else
